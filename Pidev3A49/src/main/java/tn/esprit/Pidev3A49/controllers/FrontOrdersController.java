...
public class FrontOrdersController {
    ...
    private void initialize() {
        if (!SessionRouter.ensureAuthenticated(emailSearchField)) {
            return;
        }
        ...
        try {
            ...
            String sessionEmail = resolveTrackedEmail();
            if (sessionEmail != null && !sessionEmail.isBlank()) {
                emailSearchField.setText(sessionEmail);
                emailSearchField.setEditable(false);
                searchOrders();
            } else {
                setInfoMessage("No authenticated user email was found for this session.");
                setNotificationHint("Notifications are waiting for a valid signed-in email.");
            }
    ...
    public void searchOrders() {
        ...
        String email = resolveTrackedEmail();
        if (email.isEmpty()) {
            stopNotificationPolling();
            hideNotificationsMenu();
            clearTrackedEmailState();
            renderOrders(new ArrayList<>());
            setInfoMessage("No authenticated email is available for this session.");
            setNotificationHint("Notifications need a signed-in user email.");
            return;
        }
    ...
    private String resolveTrackedEmail() {
        String sessionEmail = AppSession.getInstance().getEmail();
        if (sessionEmail != null && !sessionEmail.isBlank()) {
            return sessionEmail.trim();
        }

        String typedEmail = emailSearchField == null || emailSearchField.getText() == null
                ? ""
        ...
        String lastCheckoutEmail = CartStore.getInstance().getLastCheckoutEmail();
        return lastCheckoutEmail == null ? "" : lastCheckoutEmail.trim();
    }
