...
public final class SessionRouter {
    ...
    public static void openFrontHome(Node source) {
        try {
            FXMLLoader loader = new FXMLLoader(SessionRouter.class.getResource("/front-feed.fxml"));
            Scene scene = new Scene(loader.load());
            Stage stage = extractStage(source);
            stage.setTitle("Fitopia Community");
    ...
    public static void openSupplementStore(Node source) {
        try {
            SceneNavigator.navigate(source, SceneNavigator.FORUM_FEED_VIEW, SceneNavigator.FRONT_END_VIEW);
        } catch (IOException exception) {
            throw new IllegalStateException("Impossible d'ouvrir la page supplements.", exception);
        }
    }
    ...
    public static void logoutToSignIn(Node source) {
        CartStore.getInstance().clearSessionData();
        PendingOrderStore.getInstance().clear();
        ConfirmedOrderStore.getInstance().clear();
        UserSession.clear();
        openSignIn(source);
    }
