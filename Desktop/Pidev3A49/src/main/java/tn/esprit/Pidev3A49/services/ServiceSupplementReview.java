...
public class ServiceSupplementReview {
    ...
    public List<SupplementReview> getBySupplementId(int supplementId) {
        ...
        String query = """
                SELECT `id`, `supplement_id`, `rating`, `comment` AS `review_comment`, %s
                FROM `%s`
                WHERE `supplement_id` = ?
                %s
                """.formatted(createdAtSelection, reviewTable, orderBy);
    ...
    private String resolveReviewerName() {
        String sessionEmail = resolveReviewerEmail();
        if (sessionEmail != null && !sessionEmail.isBlank()) {
            int atIndex = sessionEmail.indexOf('@');
            String base = atIndex > 0 ? sessionEmail.substring(0, atIndex) : sessionEmail;
            if (!base.isBlank()) {
                return base;
            }
        }
