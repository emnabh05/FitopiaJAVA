...
public class MainController {
    ...
    @FXML private ComboBox<FitnessExercise> cbExerciseSelectorEdit;
    ...
    @FXML
    private void afficherModificationExercise() {
        chargerExercises();
        afficherPaneExercise(paneExerciseEdit, btnActionModifierExercise);
        tableExercisesEdit.refresh();
        if (!tableExercisesEdit.getItems().isEmpty()) {
            tableExercisesEdit.getSelectionModel().selectFirst();
            cbExerciseSelectorEdit.setValue(tableExercisesEdit.getItems().get(0));
        }
    }

    @FXML
    private void afficherSuppressionExercise() {
        chargerExercises();
        afficherPaneExercise(paneExerciseDelete, btnActionSupprimerExercise);
        tableExercisesDelete.refresh();
        if (!tableExercisesDelete.getItems().isEmpty()) {
            tableExercisesDelete.getSelectionModel().selectFirst();
        }
    }
    ...
    private void viderFormulaireExerciseModification() {
        tfExerciseId.clear();
        cbExerciseSelectorEdit.getSelectionModel().clearSelection();
    ...
    private void initialiserTableExercises() {
        ...
        tableExercises.setPlaceholder(new Label("Aucun exercice trouve."));
        tableExercisesEdit.setPlaceholder(new Label("Aucun exercice disponible pour la modification."));
        tableExercisesDelete.setPlaceholder(new Label("Aucun exercice disponible pour la suppression."));
    }
    ...
    private void initialiserComboBoxes() {
        ...
        cbExerciseSelectorEdit.setConverter(new StringConverter<>() {
            @Override
            public String toString(FitnessExercise exercise) {
                return exercise == null ? "" : exercise.getId() + " - " + exercise.getName() + " (" + exercise.getMuscleGroup() + ")";
            }

            @Override
            public FitnessExercise fromString(String string) {
                return null;
            }
        });
    ...
    private void initialiserSelections() {
        ...
        tableExercisesEdit.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                cbExerciseSelectorEdit.setValue(newValue);
                remplirFormulaireExerciseModification(newValue);
            }
        });

        cbExerciseSelectorEdit.valueProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                tableExercisesEdit.getSelectionModel().select(newValue);
                remplirFormulaireExerciseModification(newValue);
            }
        });
    ...
    private void chargerExercises() {
        ...
        cbExerciseSelectorEdit.setItems(FXCollections.observableArrayList(exercises));
